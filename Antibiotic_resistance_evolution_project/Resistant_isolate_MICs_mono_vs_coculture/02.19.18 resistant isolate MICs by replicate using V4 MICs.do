...
*Rif Ec graph
stripplot mic if ab=="Rif" & spp=="E", over(typerep) stack center vertical separate(typerep)  ///
title("{it:E. coli} Rifampicin, resistant isolate MICs by replicate", size(medlarge) color(black) margin(zero)) ///
ytitle("[Rifampicin] (ug/mL)", size(medsmall)) xtitle("Evolution condition", size(medium) margin(vsmall)) ///
height(0.5) ///
...
*Rif ESal graph
stripplot mic if ab=="Rif" & spp=="S", over(typerep) stack center vertical separate(typerep)  ///
...
plotregion(margin(zero)) ///
ylabel(#10, angle(horizontal) labsize(medsmall)) ///
ylabel(0(25)300, labsize(medium)) yscale(range(0 300)) 

*Amp Ec graph
stripplot mic if ab=="Amp" & spp=="E", over(typerep) stack center vertical separate(typerep)  ///
title("{it:E. coli} Ampicillin, resistant isolate MICs by replicate", size(medlarge) color(black) margin(zero)) ///
ytitle("[Ampicillin] (ug/mL)", size(medsmall)) xtitle("Evolution condition", size(medium) margin(vsmall)) ///
height(0.5) ///
mcolor(blue bluishgray cyan eltblue midblue teal lime*0.7 dkgreen olive_teal midgreen mint eltgreen) ///
msymbol(circle circle circle circle circle circle circle circle circle circle circle circle) ///
msize(large large large large large large large large large large large large) ///
...
*Amp Sal graph
stripplot mic if ab=="Amp" & spp=="S", over(typerep) stack center vertical separate(typerep)  ///
...
plotregion(margin(zero)) ///
ylabel(#10, angle(horizontal) labsize(medsmall)) ///
ylabel(0(50)600, labsize(medium)) yscale(range(0 600)) 
