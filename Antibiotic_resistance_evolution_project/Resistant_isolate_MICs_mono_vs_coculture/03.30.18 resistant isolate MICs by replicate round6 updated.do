...
*data is whatever the latest MIC round is (6, on 3/30/18)- with revised metadata
...
*Amp Sal graph
*NB: don't have any data for Scoop rep4 right now, so take these out: took out midgreen from mcolor and manually moved mint dot over to rep5
...
ytitle("log2[Ampicillin] (ug/mL)", size(medsmall)) xtitle("Evolution condition", size(medium) margin(vsmall)) ///
height(0.5) ///
mcolor(cranberry pink*0.5 maroon red magenta erose lime*0.7 dkgreen olive_teal mint eltgreen) ///
