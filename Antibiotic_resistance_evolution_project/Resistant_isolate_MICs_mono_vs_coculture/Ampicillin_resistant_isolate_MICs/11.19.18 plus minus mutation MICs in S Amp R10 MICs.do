...
stripplot log2mic if ab=="Amp" & dumb_shit>=7 & type=="res_isol", over(ompF) ///
 stack center vertical separate(rep) ///
...
mcolor(blue orange gold red green purple) ///
...
legend(nocolfirst rows(3) order(1 "rep1, rMM067" 2 "rep2, rMM078" 3 "rep3, rMM090" 4 "rep4, rMM098" 5 "rep5, rMM108" 6 "rep6, rMM119") size(medsmall)) ///
...
legend(nocolfirst rows(3) order(1 "rep1, rMM067" 2 "rep2, rMM078" 3 "rep3, rMM090" 4 "rep4, rMM098" 5 "rep5, rMM108" 6 "rep6, rMM119") size(medsmall)) ///
graphregion(fcolor(white)) /// 
plotregion(margin(zero)) aspectratio(0.6) ///
...
*I have not changed this one because MICs in amp P10 only changed for Smono
