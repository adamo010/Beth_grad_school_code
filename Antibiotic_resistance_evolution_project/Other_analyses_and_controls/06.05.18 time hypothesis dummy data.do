...
twoway (connected ecoop_mic2 time if time<=4, lcolor(midblue) lwidth(thick) mcolor(midblue) msymbol(circle) msize(large) mlcolor(black) mlwidth(vthin)) ///
(connected scoop_mic2 time if time<=4, lcolor(gold) lwidth(thick) mcolor(gold) msymbol(circle) msize(large) mlcolor(black) mlwidth(vthin)), ///
xlabel(, labsize(large)) xscale(range(0 4)) xtick(#5) ///
ylabel(, labsize(large) angle(horizontal)) yscale(range(0(0.5)5)) ytick(#11) ///
xtitle("Time", size(vlarge)) ///
ytitle("log{subscript:2}(MIC)", size(vlarge)) ///
...
twoway (connected ecoop_mic2 time if time<=4, lcolor(midblue) lwidth(thick) mcolor(midblue) msymbol(circle) msize(large) mlcolor(black) mlwidth(vthin)) ///
(connected scoop_mic2 time if time<=4, lcolor(gold) lwidth(thick) mcolor(gold) msymbol(circle) msize(large) mlcolor(black) mlwidth(vthin)) ///
(connected coop_mic2 time if time<=4, lcolor(midgreen) lwidth(thick) mcolor(midgreen) msymbol(circle) msize(large) mlcolor(black) mlwidth(vthin)), ///
xlabel(, labsize(large)) xscale(range(0 4)) xtick(#5) ///
ylabel(, labsize(large) angle(horizontal)) yscale(range(0(0.5)5)) ymtick(#11) ///
xtitle("Time", size(vlarge)) ///
ytitle("log{subscript:2}(MIC)", size(vlarge)) ///
legend(nocolfirst rows(2) order(1 "{it:E. coli} monoculture" 2 "{it: S. enterica} monoculture" 3 "Effective co-culture") size(large)) ///
graphregion(fcolor(white)) /// 
plotregion(margin(small)) ///
scale(0.8) aspectratio(0.6)
