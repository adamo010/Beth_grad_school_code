...
clear
use "C:\Users\adamo\OneDrive\Documents\Harcombe_lab\AbR paper 3\Xboard analysis\04.03.19 isobolograms mono and co-cultures with preds.dta"

*Only combos 8 (cipro/bleo), 1(nalx/strep), and 6 (spect/strep) used
...
title("Ciprofloxacin/ bleomycin", size(huge) color(black) margin(medium)) ///
xtitle("FIC Ciprofloxacin", size(vlarge) margin(medsmall)) ytitle("FIC Bleomycin", size(vlarge)) ///
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(0.2)1.2, angle(horizontal) labsize(vlarge)) yscale(range(0 1.2)) ///
legend(nocolfirst rows(1) order(2 "ES (obs.)" 3 "ES (pred.)") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)

*Combo 1: Nalidixic acid/ bleomycin
graph twoway (scatteri 0 1 1 0, connect(l) clwidth(thick) clcolor(black) clpattern(shortdash) msymbol(i)) ///
...
lcolor(green*0.25) lwidth(thick) mcolor(green*0.25) msymbol(cirlce) msize(large) mlcolor(black) mlwidth(vthin)), ///
title("Nalidixic acid/ streptomycin", size(huge) color(black) margin(medium)) ///
xtitle("FIC Nalidixic acid", size(vlarge) margin(medsmall)) ytitle("FIC Streptomycin", size(vlarge)) ///
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(0.2)1.2, angle(horizontal) labsize(vlarge)) yscale(range(0 1.2)) ///
legend(nocolfirst rows(1) order(2 "ES (obs.)" 3 "ES (pred.)") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)
...
graph twoway (scatteri 0 1 1 0, connect(l) clwidth(thick) clcolor(black) clpattern(shortdash) msymbol(i)) ///
(connected fic_hoz fic_vert if rep==2 & combo_num==6 & spp=="ES" & pred_or_real=="real", ///
lcolor(green) lwidth(thick) mcolor(green) msymbol(cirlce) msize(large) mlcolor(black) mlwidth(vthin)) ///
(connected fic_hoz fic_vert if rep==2 & combo_num==6 & spp=="ES" & pred_or_real=="pred", ///
lcolor(green*0.25) lwidth(thick) mcolor(green*0.25) msymbol(cirlce) msize(large) mlcolor(black) mlwidth(vthin)), ///
title("Spectinomycin/ streptomycin", size(huge) color(black) margin(medium)) ///
xtitle("FIC Spectinomycin", size(vlarge) margin(medsmall)) ytitle("FIC Streptomycin", size(vlarge)) ///
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(1)8, angle(horizontal) labsize(vlarge)) yscale(range(0 8)) ///
legend(nocolfirst rows(1) order(2 "ES (obs.)" 3 "ES (pred.)") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)
...
lcolor(green*0.25) lwidth(thick) mcolor(green*0.25) msymbol(cirlce) msize(large) mlcolor(black) mlwidth(vthin)), ///
title("Nalidixic acid/ bleomycin", size(huge) color(black) margin(medium)) ///
xtitle("FIC Nalidixic acid", size(vlarge) margin(medium)) ytitle("FIC Bleomycin", size(vlarge)) ///
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(0.2)1.2, angle(horizontal) labsize(vlarge)) yscale(range(0 1.2)) ///
legend(nocolfirst rows(1) order(2 "ES (obs.)" 3 "ES (pred.)") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)
