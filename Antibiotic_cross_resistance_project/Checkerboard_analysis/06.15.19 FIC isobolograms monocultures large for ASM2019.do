...
legend(nocolfirst rows(1) order(2 "{it:E. coli}" 3 "{it:S. enterica}") size(vlarge)) ///
...
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(0.4)2.4, angle(horizontal) labsize(vlarge)) yscale(range(0 2.4)) ///
legend(nocolfirst rows(1) order(2 "{it:E. coli}" 3 "{it:S. enterica}") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)
...
xtitle("FIC Nalidixic acid", size(vlarge) margin(medsmall)) ytitle("FIC Bleomycin", size(vlarge)) ///
xlabel(0(0.2)1.2, labsize(vlarge)) xscale(range(0 1.2)) ///
ylabel(0(0.2)1.2, angle(horizontal) labsize(vlarge)) yscale(range(0 1.2)) ///
legend(nocolfirst rows(1) order(2 "{it:E. coli}" 3 "{it:S. enterica}") size(vlarge)) ///
plotregion(margin(zero)) graphregion(fcolor(white)) aspectratio(0.5)
